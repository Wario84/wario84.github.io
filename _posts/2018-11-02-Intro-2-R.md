...
This objects have specific *attributes*: class `class()`, structure `str()`, type of `typeof()`. Unidimensional *vectors* and *lists* have `length()` which returns the number of elements, but multidimensional objects such as *matrices* and *data frames* have dimension `dim`, which return the number of rows `nrow()` and columns `ncol()`.
