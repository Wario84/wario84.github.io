...
Use parentheses `()` to group expresions only if needed. For instance, `(3 + 2)` is equivalent to `2 + 3` but the later uses less operators and it is prefered. Parentheses are used correctly to specify the order of in which **R** performs operations. For instance, `9 + 1/2` is not the same as `(9 + 1)/2`. In the former experession, `9` is added to `1/2`, but in the latter `(9 + 1)` is solved first and then divided `(9 + 1)/2`. Remember that lines are read from left to right.
