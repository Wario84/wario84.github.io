...
There are many sources that you can use to learn `R`. For instance, you can try the excellent [Introduction to R from Datacamp](https://www.datacamp.com/courses/free-introduction-to-r) but the course is not free. The Comprehensive R Archive Network (CRAN) also has a great [manual](https://cran.r-project.org/doc/manuals/r-release/R-intro.pdf) that is more comprehensive and can discourage new users. Hence, the purpose of this introduction is to teach the R-language freely and in the simplest way to encourage new users.

I have identified, through my experience of teaching R, six main building blocks of the language. New users should focus on acquiring knowledge of them to shorten the learning time. Each section covers a building block (operators, atomic elements, objects, functions, control structures and loops) with an explanation and with exercises. The exercises help you to practice and to detect common errors.
...
The operators are **special characters** that perform an **action** with a specific result. CRAN has an exhaustive list of operators that you can retrieve [here](https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Operators). But to start learning is best to focus first on the most commonly used operators, most likely you know most of them if you have used a calculator.
