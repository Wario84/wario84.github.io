...
**R** is an [object-oriented programming language](https://adv-r.hadley.nz/oo.html), which means for simplicity that everything aside the *operators* and *syntaxis* is defined an object which has a specific *attributes*: class, structure, type of, length, dimension; but also a *method*, which is what the object can do. Before we start showing some of the most common and useful objects, is important that you pay attention to the use of the *left assignment* for storing objects, the *brackets* for declaring arguments of functions, and *parentheses* and *$* for subsetting objects.
